...
export default function InterviewTemplatesManagerPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await authenticatedFetch("/api/eruhim/interview/templates", { cache: "no-store" });
  ...
  const onSelect = async (id) => {
    ...
    setMode("edit");
    try {
      const res = await authenticatedFetch(`/api/eruhim/interview/templates/${id}`);
      const data = await res.json();
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const url = editing.id
        ? `/api/eruhim/interview/templates/${editing.id}`
        : "/api/eruhim/interview/templates";
      ...
      // refresh list
      const resList = await authenticatedFetch("/api/eruhim/interview/templates", { cache: "no-store" });
  ...
  const onDelete = async (id) => {
    ...
    try {
      const res = await authenticatedFetch(`/api/eruhim/interview/templates/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
  ...
  const onSetActive = async (id) => {
    try {
      const res = await authenticatedFetch("/api/eruhim/interview/templates/active", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
